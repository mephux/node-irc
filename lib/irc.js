        ...
        modeList.forEach(function(mode) {
          ...
          if (mode in self.prefixForMode) {
            ...
            if (adding) {
              ...
              if (channel.hasOwnProperty('users')) {
...
Client.prototype.connect = function(retryCount, callback) { // {{{
  ...
  if (self.opt.secure) {
    ...
    if (typeof self.opt.secure !== 'object') {
      creds = {
        rejectUnauthorized: !self.opt.selfSigned
      };
    }
  ...
  self.conn.addListener('clientError', function() {});

  self.conn.addListener("error", function(exception) {
    self.emit("connection:error", exception);
  });
  ...
  self.conn.addListener("close", function() {
    ...
    self.emit('connection:disconnect');

    if (self.conn.requestedDisconnect) {
      return;
    }

    if (self.opt.retryCount !== null && (self.tryCount || 0) >= self.opt.retryCount) {
      self.emit('connection:abort', self.opt.retryCount, self.tryCount);
      return;
    }

    // setTimeout(function() {
    // self.tryCount++;
    // self.emit('connection:reconnect', self.tryCount);

    // self.connect(self.tryCount);
    // }, self.opt.retryDelay);
